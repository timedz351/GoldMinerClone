...
def next_level():
   global suradnice,obrazky,obr_name,typ,nazov,vyber,level,body
...
def klik(s):
   global suradnice,number,obrazky,obr_name,nazov,body
   mx= s.x
   my= s.y
...
def zapis():
      ...
      goal_zatial=(2/3)*body
      goal_list.append(int(goal_zatial))
      next_level()

def koniec():
   global done
   if level ==4:
      goals=open('goals.txt','w')
      goals.write(str(goal_list))
      goals.close()
      subor.close()
      print(suradnice)
      print(obrazky)
