...
def level1():
   ...
   ore_x=[(265,335,485,515]
   ore_y=[320,380,335,365]
...
index_ore=None
def grab():
   global catch, pokus,index_ore,which_ore
   
   for u in range(0,(len(ore_x)),2):

      if ore_x[u]<x<ore_x[u+1] and ore_y[u]<y<ore_y[u+1]:
         index_ore=int(u)
         which_ore=ore_name[int(u/2)]
         catch=1
...
def pull_up():
   ...
   if index_ore==2:
      canvas.delete(which_ore)
      canvas.create_image(x,y,image=obrazky[],anchor='center',tags='zlato2')
   if pocet<=0:
      ore_x[index_ore],ore_x[index_ore+1],ore_y[index_ore],ore_y[index_ore+1]=0,0,0,0
      catch=0
      pokus=0
      canvas.delete(which_ore)
